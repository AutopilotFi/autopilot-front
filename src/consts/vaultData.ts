...
export const getVaultDataFromAutopilots = (
  ...
  const autopilot = availableAutopilots.find(ap => ap.protocol === protocol && ap.asset === asset);
  ...
  const totalBalance =
    parseFloat(
      autopilot.vault.plasmaHistory?.[autopilot.vault.plasmaHistory.length - 1]?.totalBalance || '0'
    ) || 0;
  ...
  const vaultData: VaultOnlyData = {
    ...
    totalBalance:
      autopilot.vault.plasmaHistory?.[autopilot.vault.plasmaHistory.length - 1]?.totalBalance ||
      '0',
