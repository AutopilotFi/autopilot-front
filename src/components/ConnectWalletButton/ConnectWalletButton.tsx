...
function avatarUrl(address?: string, ensAvatar?: string | null) {
  ...
  return (
    <ConnectButton.Custom>
      {({
        ...
        const ensAvatar = (account as unknown as { ensAvatar?: string })?.ensAvatar;
        const _avatar = avatarUrl(addr, ensAvatar ?? null);
        const networkLabel =
          (chain?.id && NETWORK_LABELS[chain.id]) || chain?.name || "Connected";
        ...
        return (
          <div
            ...
            {...(!ready && {
              "aria-hidden": true,
              style: { opacity: 0, pointerEvents: "none", userSelect: "none" } as React.CSSProperties,
            })}
          >
            {/* Not connected */}
            {!connected && (
            ...
            {connected && chain?.unsupported && (
              <button
                ...
                className="w-full flex items-center space-x-3 p-3 bg-gradient-to-r from-rose-500 to-orange-500 hover:from-rose-500/90 hover:to-orange-500/90 text-white rounded-lg transition-all duration-200 font-medium shadow-md hover:shadow-lg transform hover:scale-[1.01] focus:outline-none focus:ring-2 focus:ring-offset-0 focus:ring-rose-300"
                ...
                {menuOpen && (
                  ...
                  >
                    ...
                    <button
                      ...
                      className="w-full flex items-center space-x-3 p-3 text-left hover:bg-purple-50 transition-colors last:rounded-b-lg border-t border-gray-100 group focus:outline-none"
                    >
