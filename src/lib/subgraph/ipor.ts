...
export async function fetchIporVaultHistories(
  ...
  while (vaultHistoryData.length % 1000 === 0 && vaultHistoryData.length > 0) {
    ...
    const additionalData = await executeGraphCall<Resp>(url, query, {
      vault: lc(vaultAddress),
      finishTime: lastTimestamp,
    });
...
export async function fetchIporUserBalanceHistories(
  ...
  type Resp = {
    plasmaUserBalanceHistories: Array<{
      timestamp: string | number;
      value: string;
      plasmaVault: { id: string };
      tx: string;
    }>;
  };

  let userHistoryData: Array<{
    timestamp: number;
    value: string;
    plasmaVault: { id: string };
    tx: string;
  ...
  // Initial fetch
  const data = await executeGraphCall<Resp>(url, query, {
    vault: lc(vaultAddress),
    account: lc(account),
    finishTime,
  });
...
export async function fetchVaultPublishDate(
  ...
  try {
    ...
    const minTs = hist.reduce(
      (min, h) => Math.min(min, h.timestamp || Number.MAX_SAFE_INTEGER),
...
export function aggregateUserTotals(records: PlasmaUserBalanceHistory[]): {
  ...
  const inferType = (r: PlasmaUserBalanceHistory): 'deposit' | 'withdraw' | 'earn' | 'unknown' => {
    ...
    if (a.includes('earn') || a.includes('harvest') || a.includes('reward') || a.includes('yield'))
      return 'earn';
