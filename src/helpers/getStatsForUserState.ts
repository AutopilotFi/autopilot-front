import { AllUserStats, AutopilotProduct, ProjectKey } from '@/types/globalAppTypes';
import { vaultsData } from '@/consts/autopilotData';
const availableProjects: ProjectKey[] = ['morpho-USDC', 'morpho-ETH', 'morpho-cbBTC'];

export function getStatsForUserState(selectedAutopilot: AutopilotProduct): AllUserStats {
  return {
    //mock stats for new user
    new: availableProjects.reduce(
      (acc, project) => ({
        //mock stats for all projects
        ...acc,
        [project]: {
          totalBalance: '—',
          totalEarnings: '—',
    ...
    //mock stats for old user
    old: availableProjects.reduce(
      (acc, project) => ({
        //mock stats for all projects
        ...acc,
        [project]: {
          ...
          totalDeposits:
            selectedAutopilot.protocol === 'morpho'
              ? selectedAutopilot.asset === 'USDC'
                ? '195,000'
                : selectedAutopilot.asset === 'ETH'
                  ? '82.5'
                  : '3.1'
              : selectedAutopilot.asset === 'USDC'
                ? '175,000'
    ...
    //mock stats for active user
    active: availableProjects.reduce(
      (acc, project) => ({
        //mock stats for all projects
        ...acc,
        [project]: {
            ...
            selectedAutopilot.protocol === 'morpho'
              ...
              : selectedAutopilot.asset === 'USDC'
                ...
                : selectedAutopilot.asset === 'ETH'
                  ? '1.8'
                  : '0.12',
          totalActions: selectedAutopilot.protocol === 'morpho' ? '6' : '52',
          transactions:
            project === 'morpho-USDC'
              ? [
                  {
